...
- 逐句重练：结果页可针对单句重新听写，沿用同一套逐词评分；每句保留最近 3 次重练（新到旧），不影响原课程成绩与教师反馈。
- 掌握状态：单句连续两次重练满分标记为已掌握，最近一次低于满分即回到待巩固；课程列表显示每课待巩固句数。
...
- 记录导出：将练习历史、逐词结果、错误分类、原因、教师反馈、逐句重练记录与掌握状态导出为 JSON。
...
浏览器存储键为 `sologsb-1029-dictation-state-v1`（schemaVersion 2，旧版本数据会自动迁移并保留原有记录）。重练记录按句保存最近 3 次，掌握状态由重练成绩推导：连续两次满分为已掌握，最近一次低于满分为待巩固。导出文件为 UTF-8 JSON，包含 `attempts`、`repractices`、`mastery` 与 `progress`，可用于后续学习分析。
